"""
Background Drive processor (Pakistan deployment).
Polls a Google Drive folder for new invoices, processes them via OpenAI,
applies GST tax codes (Pakistan), and pushes results to Sheets + QBO.
"""
import asyncio
import os
import uuid
    ...
    def _process_file(self, file_info: dict):
        ...
        try:
            ...
            if os.getenv("DUPLICATE_CHECK_ENABLED", "true").lower() == "true":
                is_dup = self.sheets.check_duplicate(result.invoice_number, result.supplier_name)
                if is_dup:
            ...
            if self.qbo:
                try:
                    ...
                    invoice_dict["file_id"] = internal_id

                    # ── GST Processing (Pakistan: per-line tax codes only, no RCM) ──
                    from services.gst_processor import process_gst
                    invoice_dict = process_gst(invoice_dict)
                    # ── End GST Processing ───────────────────────────
